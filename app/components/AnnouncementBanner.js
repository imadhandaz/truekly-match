...
const slides = [
  ...
  { gradient: "linear-gradient(135deg,#f97316 0%,#ef4444 100%)", emoji: "🌴", tag: "INTERCAMBIO DE CASAS", title: "Madrid ↔ Málaga", subtitle: "Vives en Madrid y quieres playa — otro quiere ciudad. ¡Trueká tu casa!" },
  { gradient: "linear-gradient(135deg,#0ea5e9 0%,#6366f1 100%)", emoji: "🏡", tag: "VACACIONES GRATIS", title: "Tu casa por la suya", subtitle: "Barcelona, Valencia, Sevilla, Bilbao... ¡sin pagar hotel!" },
  { gradient: "linear-gradient(135deg,#10b981 0%,#84cc16 100%)", emoji: "🏖️", tag: "VERANO", title: "Apartamento en la playa", subtitle: "Intercambia tu piso por una semana junto al mar" },
  { gradient: "linear-gradient(135deg,#374151 0%,#1f2937 100%)", emoji: "🚗", tag: "COCHES Y MOTOS", title: "Intercambia tu coche", subtitle: "Busca tu próximo vehículo haciendo trueque" },
  ...
  { gradient: "linear-gradient(135deg,#16a34a 0%,#15803d 100%)", emoji: "⚽", tag: "ENCUENTRA EQUIPO", title: "Pachanga de fútbol", subtitle: "¿Te faltan jugadores? Encuentra tu once en Madrid" },
  ...
  { gradient: "linear-gradient(135deg,#0e7490 0%,#1d4ed8 100%)", emoji: "🏊", tag: "NATACIÓN · SURF", title: "Deportes acuáticos", subtitle: "Encuentra con quien practicar surf, kayak o piscina" },
];
      ...
      <button onClick={() => onSlideClick?.()} className="relative w-full overflow-hidden rounded-2xl text-left transition-all active:scale-[0.98]"
        ...
        <div className="absolute inset-0 flex items-center px-5 gap-4"
          ...
          <div className="flex-1 min-w-0">
            <span className="inline-block font-black mb-0.5" style={{ fontSize:9, color:"rgba(255,255,255,0.75)", letterSpacing:"0.12em" }}>{slide.tag}</span>
            <p className="text-white font-black text-lg leading-tight truncate">{slide.title}</p>
            <p style={{ color:"rgba(255,255,255,0.8)", fontSize:11 }} className="leading-snug mt-0.5 line-clamp-2">{slide.subtitle}</p>
          </div>
        ...
        <div className="absolute bottom-2.5 left-0 right-0 flex justify-center gap-1">
          {slides.map((_, i) => (<span key={i} style={{ display:"inline-block", width: i===current ? 14 : 4, height:4, borderRadius:2, background: i===current ? "white" : "rgba(255,255,255,0.3)", transition:"all 0.3s ease" }} />))}
        </div>
      </button>
    </div>
  );
