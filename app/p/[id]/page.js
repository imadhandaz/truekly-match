...
export async function generateMetadata({ params }) {
  ...
  const desc = owner + " ofrece " + p.title + " a cambio de " + p.wants + ". ¡Únate al trueque!";
  ...
  return {
    ...
    openGraph: {
      ...
      url: process.env.NEXT_PUBLIC_APP_URL + "/p/" + id,
      type: "website",
    },
    twitter: {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="relative rounded-3xl overflow-hidden shadow-2xl mb-6" style={{ aspectRatio: "3/4" }}>
          {photo ? (
            <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: "url(" + JSON.stringify(photo) + ")" }} />
          ) : (
        ...
        <p className="text-center text-xs text-foreground/50 mt-3">Trueque · Intercambio · Sin dinero</p>
      </div>
    </div>
  );
}
