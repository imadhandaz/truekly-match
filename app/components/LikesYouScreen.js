    ...
    <div className="w-full max-w-md animate-fadeIn">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-2xl font-bold bg-gradient-to-r from-yellow-500 via-orange-500 to-yellow-600 bg-clip-text text-transparent">
          {products.length} {products.length === 1 ? "persona te ha dado like" : "personas te han dado like"}
        </h2>
      </div>
      ...
      {!isGold && (
        ...
        >
          <div className="text-left">
            ...
            <p className="text-xs font-medium opacity-90 mt-0.5">Y matchea hasta 3× más rápido</p>
          </div>
      ...
      {products.length === 0 ? (
        ...
        <div className="grid grid-cols-2 gap-3">
          {products.map((p) =>
            isGold ? (
              <div key={p.id} className="relative aspect-[3/4] rounded-2xl overflow-hidden shadow-lg">
                <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url('${p.photos?.[0]}')` }} />
              ...
              <button key={p.id} onClick={onUpgrade} className="relative aspect-[3/4] rounded-2xl overflow-hidden shadow-lg group hover:scale-[1.02] transition">
                <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url('${p.photos?.[0]}')`, filter: "blur(18px)" }} />
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-black/40" />
                <div className="absolute inset-0 flex flex-col items-center justify-center text-white">
                  <div className="text-3xl mb-1">🔒</div>
                  <p className="text-xs font-bold uppercase tracking-wider">Bloqueado</p>
                </div>
                <div className="absolute top-2 right-2 px-2 py-1 rounded-full bg-gradient-to-r from-yellow-400 to-orange-500 text-white text-[9px] font-black shadow">GOLD</div>
              </button>
            )
          )}
        </div>
